...
export default function Home() {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        ...
        transcript: [...updatedMessages]
          .map(m => `${m.role === 'user' ? 'You' : 'Thryve'}: ${m.content}`)
          .join('\n')
      })
    });
  ...
  useEffect(() => {
    setIsTyping(true);
    setTimeout(() => {
      setMessages([{ role: 'assistant', content: 'Hey! 👋 Want our FREE 24-Hour Inquiry Removal Guide? I’ll send it to you — just drop your first name to start.' }]);
      setIsTyping(false);
    }, 1500);
  }, []);
